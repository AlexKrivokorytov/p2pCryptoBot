...
class SupportedAsset(str, enum.Enum):
...
class Order(Base):
    ...
    taker_id: Mapped[int | None] = mapped_column(
    ...
    asset: Mapped[str] = mapped_column(
        Enum(SupportedAsset, name="supported_asset"), nullable=False
    )
    ...
    invoice_id: Mapped[str | None] = mapped_column(String(128), nullable=True, unique=True)
    spend_id: Mapped[str | None] = mapped_column(String(128), nullable=True, unique=True)
    crypto_pay_payload: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    taker: Mapped["User | None"] = relationship(  # type: ignore[name-defined]  # noqa: F821
        "User", foreign_keys=[taker_id], lazy="selectin"
    )

    def __repr__(self) -> str:
        return f"<Order id={self.id} type={self.order_type} asset={self.asset} status={self.status}>"
