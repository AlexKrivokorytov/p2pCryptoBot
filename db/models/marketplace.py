...
class UserPaymentDetail(Base):
    ...
    payment_method_id: Mapped[int] = mapped_column(ForeignKey("payment_methods.id", ondelete="CASCADE"), nullable=False)
...
class Ad(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    maker_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id", ondelete="CASCADE"), nullable=False)
    ...
    price_type: Mapped[PriceType] = mapped_column(Enum(PriceType, create_type=False), nullable=False)
    price_value: Mapped[float] = mapped_column(Numeric(precision=18, scale=2), nullable=False) # Fixed price or % margin
    
    min_limit: Mapped[float] = mapped_column(Numeric(precision=18, scale=2), nullable=False) # In Fiat
    max_limit: Mapped[float] = mapped_column(Numeric(precision=18, scale=2), nullable=False) # In Fiat
    
    payment_method_ids: Mapped[str] = mapped_column(String(255), nullable=False) # Comma-separated IDs
...
class Review(Base):
    ...
    reviewer_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id", ondelete="CASCADE"), nullable=False)
    target_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id", ondelete="CASCADE"), nullable=False)
...
class ReferralReward(Base):
    ...
    referred_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id", ondelete="SET NULL"), nullable=True)
...
class DisputeTicket(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    order_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("orders.id", ondelete="CASCADE"), nullable=False, unique=True)
    creator_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id", ondelete="CASCADE"), nullable=False)
    moderator_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.telegram_id", ondelete="SET NULL"), nullable=True)
    
    reason: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(String(32), default="open", nullable=False) # open, claimed, resolved
