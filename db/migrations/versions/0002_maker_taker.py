...
def upgrade() -> None:
    ...
    with op.get_context().autocommit_block():
        op.execute(
            "ALTER TYPE order_status ADD VALUE IF NOT EXISTS 'pending_funding' BEFORE 'escrow_held'"
        )
    ...
    op.add_column(
        ...
        sa.Column(
            "order_type",
            sa.Enum("sell_crypto", "buy_crypto", name="order_type", create_type=False),
            nullable=True,
        ),
    )
...
def downgrade() -> None:
    ...
    # Note: We do not drop the enum values for order_status
    # because Postgres doesn't support it natively.
